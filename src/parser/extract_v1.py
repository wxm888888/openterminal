#!/usr/bin/env python3
"""
V1 Format Cast File Parser

Extracts terminal interactions from v1 format cast files (standard JSON with stdout array).
Converts to the same turn-based format as v2 parser for consistency.
"""
...
def parse_v1_cast(file_path: str) -> Tuple[Dict, List[Dict]]:
    """
    Parse v1 format cast file.
    ...
    for frame in data.get('stdout', []):
        if isinstance(frame, list) and len(frame) >= 2:
            ...
            events.append({
                ...
                'type': 'o',  # v1 only has output events
                'data': content
            })
...
def calculate_confidence(command: str, prompt: str = '', is_input_derived: bool = False) -> float:
    ...
    # Boost for valid command patterns
    ...
    if prompt:
        # Strong prompt patterns (user@host:path$)
...
def verify_extraction(initial_output: str, turns: List[Dict], txt_content: str) -> Dict[str, Any]:
    ...
    for turn in turns:
        if 'action' in turn and 'content' in turn['action']:
            parts.append(turn['action']['content'] + '\n')
    ...
    # Use quick_ratio for large files to avoid performance hit
...
def extract_turns_from_v1(metadata: Dict, events: List[Dict]) -> Tuple[str, List[Dict], str]:
    """
    Extract turns from v1 events.
    
    Since v1 only has output events, we need to infer command boundaries
    from prompt patterns and output structure.
    ...
    # Combine all output - this is the complete raw data
    raw_all_output = ''.join(e['data'] for e in events)
    ...
    for i, match in enumerate(matches):
        ...
        # NEW: Skip if command doesn't look valid (whitelist check)
        ...
        # This ensures observation includes the next prompt, matching Input-based behavior
        ...
        cleaned_observation = process_backspaces(clean_output_for_display(observation_raw)).strip()
        
        turns.append({
            'turn_id': turn_id,
            'timestamp': 0,  # v1 doesn't track input timing
            'action': {
                'type': 'command',
                'raw_input': command + '\n',
...
def extract_to_turn_based_v1(file_path: str, output_path: Optional[str] = None) -> Dict:
    """
    Main extraction function for v1 format.
    ...
    """
    metadata, events = parse_v1_cast(file_path)
    initial_output, turns, raw_all_output = extract_turns_from_v1(metadata, events)
    
    # v1 only has output events
    ...
    result = {
        ...
        'turns': turns,
        'verification': {'verified': False},  # Default verification status
        'extraction_notes': {
            'method': 'v1_output_only',
            'warning': 'v1 format only contains output events. Commands are inferred from prompt patterns.'
        }
    }
    ...
    if txt_path:
        try:
            ...
            # Don't fail extraction if verification fails
    ...
    print(f"V1 extraction complete: {output_path}")
    print(f"  - Total events: {output_events} (input: 0, output: {output_events})")
...
def process_directory(target_path: str):
    ...
    for entry in os.scandir(target_path):
        if entry.is_file() and entry.name.endswith('.cast'):
            try:
                extract_to_turn_based_v1(entry.path)
            except Exception as e:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python extract_v1.py <cast_file_or_directory>")
        sys.exit(1)
    ...
    if os.path.isfile(target_path):
        extract_to_turn_based_v1(target_path)
    else:
